...
/// </summary>
public class DeathChest : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("DeathChest requires an Interactable component!");
        }
    }
    ...
    private void Start()
    {
        ...
        {
            inventoryManager = ServiceLocator.Instance.Get<IInventorySystem>();
        }
    ...
    public void Initialize(List<ItemStack> items)
    {
        ...
        Debug.Log($"DeathChest initialized with {storedItems.Count} item stacks");
        UpdateInteractionMessage();
    }
    ...
    private void OnChestInteract(GameObject interactor)
    {
        ...
        if (inventoryManager == null && ServiceLocator.Instance.IsRegistered<IInventorySystem>())
        {
            inventoryManager = ServiceLocator.Instance.Get<IInventorySystem>();
        }
        ...
        {
            Debug.LogError("Could not access player inventory from InventorySystem");
            return;
        }

        var inv = inventorySystem.PlayerInventory;
